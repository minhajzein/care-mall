...
function ProductCard({ product }) {
	return (
		<div className='md:w-full bg-white p-3 rounded w-[170px] shadow-black/50 shadow-md font-nunito gap-[2px] flex flex-col'>
			<div className='w-full relative'>
				<Link className='w-full' href={`/products/${product.title}`}>
					<img
						className='border border-primary/50 rounded h-40 w-full object-contain'
			...
			<h1 className='font-bold text-sm'>₹{product.discountPrice}</h1>
			...
			<button className='uppercase border text-xs border-primary rounded-lg mt-1 p-1 text-nowrap text-red-500 font-bold'>
				add to cart
			</button>
		</div>
	)
